...
export class CartService {
  ...
  removeItemQuantityCart(item: CartItem): void {
    ...
    if (itemForRemoval){
      filteredItems = this.removeItemCart(itemForRemoval, false);
  ...
  removeItemCart(item: CartItem, update=true): Array<CartItem> {
    const filteredItems = this.cart.value.items.filter((_item) => _item.id !== item.id);
    if (update){
      this.cart.next({items: filteredItems});
      this._snackBar.open('1 Item is Removed', 'Ok', {duration: 3000});
    }
